...
UNIT = [
    ...
    ("add try except to this code: def divide(a, b):\n    return a / b",
     "add_errors", ["try:", "except"], []),
    ("wrap this in a try catch: function divide(a, b) {\n    return a / b;\n}",
     "add_errors", ["try {", "catch"], []),
    ("make this code handle errors: def f():\n    return int(input())",
     "add_errors", ["try:", "except"], []),
    ("add a docstring to this code: def add(a, b):\n    return a + b",
     "add_docstring", ["\"\"\""], []),
    ("optimize this: result = []\nfor i in range(100):\n    result.append(i*2)",
     "make_faster", ["[i*2 for i in range(100)]"], []),
    ...
    ("convert this python code to javascript: squares = [x * x for x in items]",
     "convert_lang", ["items.map(x => x * x)"], ["for x in items"]),
    ("convert this python code to javascript: return [n for n in nums if n % 2 == 0]",
     "convert_lang", [".filter(n => n % 2 === 0).map(n => n)"], []),
    ("convert this python code to javascript: class Dog:\n    def __init__(self, name):\n        self.name = name\n\n    def bark(self):\n        print(self.name + ' says woof')",
     "convert_lang", ["constructor(name)", "this.name", "bark() {"],
     ["function constructor", "self.name"]),
    ("convert this javascript code to python: const d = items.map(x => x * 2)",
     "convert_lang", ["[x * 2 for x in items]"], [".map("]),
    ...
    # "change" must work as a synonym for "rename"
    ("change the variable x to y in this code: x = 5\nprint(x)",
     "rename", ["y = 5", "print(y)"], ["print(x)"]),
    ("change x to y in this code: x = 5\nprint(x)",
     "rename", ["y = 5", "print(y)"], ["print(x)"]),
    ("change foo to bar in this code: foo = 1\nprint(foo)",
     "rename", ["bar = 1", "print(bar)"], ["print(foo)"]),
    ("change name to username in this code: print(name)",
     "rename", ["print(username)"], ["print(name)"]),
    ("change myvariable to my_var in this code: myvariable = 1\nprint(myvariable)",
     "rename", ["my_var = 1"], ["print(myvariable)"]),
...
E2E = [
    ...
    ("what is the capital of france",
     ["paris"], []),
    # "change" without a code context is not a rename: colors, pronouns,
    # and function words are never identifiers
    ("change the color to green in this code: x = 5", ["color"], []),
    ("change this to that in this code: x = 5", ["this"], []),
    ("change red to blue in this code: x = 5", ["red"], []),
]
...
MULTITURN = [
    ...
    ("fix this code: def greet(name):\n    print(\"hello \" + name)",
     "change name to person in the last code", ["def greet(person)", "print(\"hello \" + person)"]),
]
