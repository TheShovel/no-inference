...
CASES = [
    ...
    # ── Wave 3: quality checks (must not return a plausible-but-wrong answer) ─
    ("how to check python version", ["version"]),
    ("javascript random number between 1 and 10", ["random", "math.random"]),
    ("python while loop vs for loop", ["while", "for"]),
    ("what does git pull do", ["git pull"]),
    ("how to rename a git branch", ["rename", "branch"]),
    ("what is a git stash", ["stash"]),
    ("how to find the largest number in a list python", ["max"]),
    ("python round to 2 decimal places", ["round", "decimal"]),
    ("what is a callback function", ["callback"]),
    ("how to write json to a file in python", ["json.dump"]),
    ("what is the difference between null and undefined", ["undefined", "null"]),
    ("how do cookies work in web development", ["cookie"]),
    ("what is a foreign key", ["foreign key"]),
    ("python check if string contains substring", ["in", "substring"]),
    ("how to split a string in python", ["split"]),
    ("python how to convert string to int", ["int"]),
    ("how to remove duplicates from a list in python", ["duplicate"]),
    ("what is the difference between break and continue", ["break", "continue"]),
    ("how to check if a key exists in a python dictionary", ["key"]),
    ("javascript how to sort an array of objects", ["sort"]),

    # ── Wave 4: languages, debugging, frameworks ──────────────────────────
    ("how do i declare a variable in c++", ["int", "variable"]),
    ("what is a pointer in c", ["pointer", "address"]),
    ("how to write a for loop in java", ["for"]),
    ("what is a goroutine in go", ["goroutine"]),
    ("rust ownership explained", ["ownership"]),
    ("why does my python code throw a keyerror", ["keyerror", "get"]),
    ("what is a segfault", ["segfault", "memory"]),
    ("what is a race condition", ["race", "lock"]),
    ("what is a null pointer exception", ["null"]),
    ("what is django", ["django", "framework"]),
    ("what does the mod operator do", ["modulo", "remainder"]),
    ("what is the difference between react and vue", ["react", "vue"]),
    ("how does json stringify and parse work in javascript", ["json.stringify", "json.parse"]),
    ("what is the difference between synchronous and asynchronous", ["synchronous", "asynchronous"]),
    ...
    # ── Wave 6: routing correctness (wrong-topic matches must not win) ────
    ("write a python script to check if a website is up",
     ["urllib", "status", "is_up"], ["key exists", "dictionary"]),
    ("check if a url is down", ["status"], ["key exists"]),
    ("write a python script to send an email", ["smtplib", "SMTP"], ["key exists"]),
    ("count the number of characters in a string",
     ["len", "character"], ["COUNT(*)", "word frequency"]),
    ("get the length of a string in python", ["len"], []),
    ("how to extract phone numbers from text with regex",
     ["findall", "extract_phones"], ["extract (essence)", "solvent"]),
    ("how to kill a process on a port",
     ["lsof", "kill"], ["kill is a command", "maritime"]),
    ("can you write me a function that reverses a string",
     ["def reverse_string"], ["entirely rule-based"]),
    # ── Wave 7: common tasks added via data/knowledge/code_tasks/ ─────────
    ("how to delete a file in python", ["os.remove", "unlink"], []),
    ("how to copy a file in python", ["shutil.copy"], []),
    ("how to check if a file exists in python", ["exists"], []),
    ("how to get a list of files in a directory in python", ["os.listdir"], []),
    ("write a function to find the intersection of two lists",
     ["intersection", "set"], []),
    ("how to check if a list is empty in python", ["not", "empty"], []),
    ("how to convert a list to a string in python", ["join"], []),
    ("how to read command line arguments in python", ["argv", "argparse"], []),
    ("write a function to check for leap year", ["leap", "% 4"], []),
    ("regex to match an IP address", ["ipv4"], ["email"]),
    ("how to pass arguments to a bash script", ["$1", "$@"], []),
    ("how to zip a folder in python", ["shutil.make_archive", "zipfile"], []),
    ("how to convert a string to lowercase in python", ["lower"], []),
    ("write a python script that backs up a directory",
     ["back up", "timestamped"], []),
    ("write a function to check if a number is even", ["% 2"], ["prime"]),
    ("write a function to calculate the power of a number", ["**"], []),
    ("write a function to find the median of a list", ["median"], ["max()"]),
    ("write a function to generate a random number in python",
     ["random"], []),
]
