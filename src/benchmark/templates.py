...
**Implications:**
Quantum mechanics isn't just theoretical — it's the basis for transistors, lasers, MRI machines, and the ongoing revolution in quantum computing.""",
}


def match_stem(query):
    """Match STEM/explanation queries."""
    q = query.lower()
    if any(w in q for w in ['quantum', 'superposition', 'entanglement']):
        return STEM_TEMPLATES['quantum']
    return None
