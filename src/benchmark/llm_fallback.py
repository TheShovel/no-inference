...
to the symbolic context_extraction module from the cos package.
...
# Import from the cos package
_ctx_extraction = None

def _get_ctx_extraction():
    """Lazy-import the context extraction module."""
    global _ctx_extraction
    if _ctx_extraction is None:
        ...
        _ctx_extraction = {
            ...
            'extract_topic': _extract_topic_safe,
            'extract_content_words': extract_content_words,
        }
    return _ctx_extraction
...
def extract_search_terms(query):
    ...
    ctx = _get_ctx_extraction()
    keywords = ctx['extract_keywords'](query, max_keywords=5)
...
def extract_topic(query):
    ...
    """
    ctx = _get_ctx_extraction()
    topic, confidence = ctx['extract_topic'](query)
...
def _extract_context_words(text, max_words=10):
    ...
    ctx = _get_ctx_extraction()
    words = ctx['extract_content_words'](text, max_words=max_words * 2)
...
def _is_garbage(text):
    """Check if a generated response is garbage."""
...
def synthesize_response(query, information):
    ...
    ctx = _get_ctx_extraction()
    topic, _ = ctx['extract_topic'](query)
